...
var QUERY_FIELD_PREFIX = 'data';
var QUERY_SEARCHABLE_PREFIX = 'searchable';
...
var Field = module.exports = function Field(model, query, sorts, index) {
  var index = index;
  var path = query[QUERY_COLUMNS][index][QUERY_FIELD_PREFIX];
  ...
  Object.defineProperty(this, "model", {
    writable: false, enumerable: true, value: info.model });
...
return query[QUERY_COLUMNS][index][QUERY_SEARCHABLE_PREFIX] === 'true' ||
    query[QUERY_COLUMNS][index][QUERY_SEARCHABLE_PREFIX] === true;
}
...
function loadSchemaInfo(model, info, schema, path) {
  ...
  if (schemaField) {
    info.model = info.model || model.modelName;
    ...
    if (schemaField.options.ref) {
      ...
      if (schemaField.caster && schemaField.caster.options &&
        ...
        info.model = info.ref = schemaField.caster.options.ref;
      } else {
...
function loadPopulatedSchemaInfo(model, info, schema, path) {
  ...
  var baseType = (baseField.options && baseField.options.ref) ||
		(baseField.caster.options && baseField.caster.options.ref);
