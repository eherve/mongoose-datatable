...
function buildBooleanCondition(locale, field, search, regexp, options) {
  locale = locale || 'default';
  var opt = options[locale] || options['default'];
  if ('object'== typeof opt) {
    var trueRegExp = util.isRegExp(opt.True) ? opt.True : undefined;
    var falseRegExp = util.isRegExp(opt.False) ? opt.False : undefined;
      /^false$/i;
    if (undefined != trueRegExp && trueRegExp.test(search)) return true;
    if (undefined != falseRegExp && falseRegExp.test(search)) return false;
  }
  return;
}

function buildDateCondition(locale, field, search, regexp, options) {
  locale = locale || 'default';
  var opt = options[locale] || options['default'];
  if ('object'== typeof opt && opt.format) {
    // TODO parse date
  }
  return;
}
