...
function countAllRecords(searchCriteria, resultHolder, callback) {
  this.countDocuments(searchCriteria.options.conditions || {}, function(err, count) {
    if (err) return callback(err);
...
function countFilteredRecords(searchCriteria, resultHolder, callback) {
  ...
  this.countDocuments(searchCriteria.conditions, function(err, count) {
    if (err) return callback(err);
    ...
    (function run(index) {
      if (index >= searchCriteria.populate.length) {
        ...
        return callback(null, searchCriteria, resultHolder);
			...
			self.populate(records, searchCriteria.populate[index],
				function (err, d) {
					if (err) { return callback(err); }
					records = d;
					run(++index);
